...
# DB_SIZE
    if name == 'dbsize':
        CMD = "select DB_SIZE from MON.DB_SIZE_HIST where SNAP_DATE = (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 1 ROWS ONLY)"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
# DB_SIZE_yesterday		
    if name == 'dbsizeytd':
        CMD = "select DB_SIZE from MON.DB_SIZE_HIST where SNAP_DATE = (select * from (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 2 ROWS ONLY) order by SNAP_DATE asc FETCH FIRST 1 ROWS ONLY)"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
# DB_SIZE_diff
    if name == 'dbsizediff':
        CMD = "select ((select DB_SIZE from MON.DB_SIZE_HIST where SNAP_DATE = (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 1 ROWS ONLY)) - (select DB_SIZE from MON.DB_SIZE_HIST where SNAP_DATE = (select * from (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 2 ROWS ONLY) order by SNAP_DATE asc FETCH FIRST 1 ROWS ONLY))) as TEST_FS from MON.DB_SIZE_HIST FETCH FIRST 1 ROWS ONLY"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
# DB_FS_FREE
    if name == 'dbfsfree':
        CMD = "select DB_FS_FREE from MON.DB_SIZE_HIST where SNAP_DATE = (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 1 ROWS ONLY)"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
# MIN_FREE_SIZE_ON_NODE
    if name == 'minfsfree':
        CMD = "select MIN_FREESIZE_ON_NODE from MON.DB_SIZE_HIST where SNAP_DATE = (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 1 ROWS ONLY)"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
# DATA_FS_FREE
    if name == 'datafsfree':
        CMD = "select DATA_FS_FREE from MON.DB_SIZE_HIST where SNAP_DATE = (select SNAP_DATE from MON.DB_SIZE_HIST order by SNAP_DATE desc FETCH FIRST 1 ROWS ONLY)"
        out = db2query(CMD)
        out = out[-4]
        print out.strip()
